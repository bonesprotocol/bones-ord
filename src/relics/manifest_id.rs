...
impl ManifestId {
  pub fn new(block: u64, tx: u32) -> Option<ManifestId> {
    let id = ManifestId { block, tx };
  ...
  pub fn delta(self, next: ManifestId) -> Option<(u128, u128)> {
  ...
  pub fn next(self: ManifestId, block: u128, tx: u128) -> Option<ManifestId> {
    ManifestId::new(
...
mod tests {
  ...
  fn delta() {
    let mut expected = [
      ManifestId { block: 3, tx: 1 },
      ManifestId { block: 4, tx: 2 },
      ManifestId { block: 1, tx: 2 },
      ManifestId { block: 1, tx: 1 },
      ManifestId { block: 3, tx: 1 },
      ManifestId { block: 2, tx: 0 },
    ];
    ...
    let mut previous = ManifestId::default();
  ...
  #[test]
  fn display() {
    assert_eq!(ManifestId { block: 1, tx: 2 }.to_string(), "1:2");
  }

  #[test]
  fn from_str() {
    assert!(matches!("123".parse::<ManifestId>(), Err(Error::Separator)));
    assert!(matches!(":".parse::<ManifestId>(), Err(Error::Block(_))));
    assert!(matches!(
      "1:".parse::<ManifestId>(),
      Err(Error::Transaction(_))
    ));
    assert!(matches!(":2".parse::<ManifestId>(), Err(Error::Block(_))));
    assert!(matches!("a:2".parse::<ManifestId>(), Err(Error::Block(_))));
    assert!(matches!(
      "1:a".parse::<ManifestId>(),
      Err(Error::Transaction(_)),
    ));
    assert_eq!(
      "1:2".parse::<ManifestId>().unwrap(),
      ManifestId { block: 1, tx: 2 }
    );
  }
  ...
  #[test]
  fn serde() {
    let rune_id = ManifestId { block: 1, tx: 2 };
    let json = "\"1:2\"";
    assert_eq!(serde_json::to_string(&rune_id).unwrap(), json);
    assert_eq!(serde_json::from_str::<ManifestId>(json).unwrap(), rune_id);
  }
}
