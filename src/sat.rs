...
mod tests {
  ...
  fn subsidy_position() {
    ...
    assert_eq!(
      Sat((Height(0).subsidy() as u128 - 1) as u64).third(),
      Height(0).subsidy() - 1
    );
    assert_eq!(Sat(Height(0).subsidy() as u128 as u64).third(), 0);
    assert_eq!(Sat((Height(0).subsidy() as u128 + 1) as u64).third(), 1);
    assert_eq!(
