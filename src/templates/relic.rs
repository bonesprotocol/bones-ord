...
impl Default for RelicEntryHtml {
  fn default() -> Self {
    RelicEntry::default().into()
  }
}

impl From<RelicEntry> for RelicEntryHtml {
  fn from(entry: RelicEntry) -> Self {
    // note: the entry.owner_sequence_number is omitted on purpose
    RelicEntryHtml {
      block: entry.block,
