use {
  super::*,
  crate::{
    index::syndicate_entry::SyndicateEntry,
    relics::{Enshrining, RelicId, SyndicateId},
    templates::relic::RelicEntryHtml,
  },
};

#[derive(Debug, Serialize, Deserialize)]
pub struct SyndicateEntryHtml {
  pub summoning: Txid,
...
impl From<SyndicateEntry> for SyndicateEntryHtml {
  fn from(entry: SyndicateEntry) -> Self {
    // note: the entry.sequence_number is omitted on purpose
    SyndicateEntryHtml {
      summoning: entry.summoning,
...
pub struct SyndicateHtml {
  pub entry: SyndicateEntryHtml,
  pub id: SyndicateId,
  pub chestable: bool,
  pub owner: Option<InscriptionId>,
  pub treasure: RelicEntryHtml,
}
...
impl PageContent for SyndicateHtml {
  fn title(&self) -> String {
    format!("Syndicate {}", self.entry.treasure)
  }
}
