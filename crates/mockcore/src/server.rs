...
impl Api for Server {
  ...
  fn create_raw_transaction(
    ...
    let tx = Transaction {
      ...
      output: outs
        .values()
        .map(|amount| TxOut {
          value: Amount::from_sat((*amount * COIN_VALUE as f64) as u64).to_sat(),
  ...
  fn fund_raw_transaction(
    ...
    let mut cursor = std::io::Cursor::new(hex::decode(tx).unwrap());
    ...
    let output_value = transaction.output.iter().map(|txout| txout.value).sum();
    ...
    if change > 0 {
      transaction.output.push(TxOut {
        value: Amount::from_sat(change).to_sat(),
        script_pubkey: state.new_address(true).script_pubkey(),
      });
    }
    ...
    let fee = if let Some(fee_rate) = options.fee_rate {
      ...
      transaction.output.last_mut().unwrap().value -= Amount::from_sat(fee).to_sat();
      fee
    } else {
      0
    };
  ...
  fn send_to_address(
    ...
    let mut transaction = Transaction {
      ...
      output: vec![
        ...
        TxOut {
          value: utxo_amount.to_sat() - value.to_sat(),
          script_pubkey: address.script_pubkey(),
        },
      ],
    };
    ...
    transaction.output[1].value -= Amount::from_sat(fee).to_sat();
          ...
          serde_json::to_value(GetRawTransactionResult {
            ...
            vout: transaction
              ...
              .map(|(n, output)| GetRawTransactionResultVout {
                ...
                script_pub_key: GetRawTransactionResultVoutScriptPubKey {
                  asm: to_asm_string(&output.script_pubkey),
                  hex: output.script_pubkey.clone().serialize(),
                  req_sigs: None,
                  type_: None,
                  addresses: Vec::new(),
                  address: None,
  ...
  fn list_transactions(
    ...
    Ok(
      state
        ...
        .map(|(txid, tx)| (*txid, tx))
        .chain(state.mempool.iter().map(|tx| (compute_txid(tx), tx)))
...
pub(crate) fn to_asm_string(script: &Script) -> String {
  let mut buf = String::new();
  script.fmt_asm(&mut buf).unwrap();
  buf
}
