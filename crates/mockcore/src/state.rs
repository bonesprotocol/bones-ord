...
impl State {
  ...
  pub fn balances(&self) -> BTreeMap<Address, Vec<(OutPoint, Amount)>> {
    ...
    for (&outpoint, &amount) in &self.utxos {
      ...
      if tx_out.script_pubkey == Script::new() {
        continue;
      }
  ...
  pub(crate) fn mine_block(&mut self, subsidy: u64) -> Block {
        ...
        value: subsidy
          + self
            ...
            .map(|tx| {
              ...
              self.transactions.insert(compute_txid(tx), tx.clone());

              fee
            })
            .sum::<u64>(),
        script_pubkey: self.new_address(false).script_pubkey(),
      }],
    };

    self
      .transactions
      .insert(compute_txid(&coinbase), coinbase.clone());
    ...
    for tx in block.txdata.iter() {
      self
        ...
        .insert(compute_txid(tx), self.hashes.len().try_into().unwrap());
      ...
      total_value += self
        ...
        .or_else(|| {
          ...
          self
            .mempool
            .iter()
            .find(|tx| compute_txid(tx) == outpoint.txid)
            .map(|tx| {
              tx.output
      ...
      output: (0..template.outputs)
        .map(|i| TxOut {
          ...
          script_pubkey: if let Some(recipient) = &template.recipient {
            ...
            Script::new_v1_p2tr(&secp, internal_key.0, None)
          } else {
            Script::new_v0_p2wpkh(&WPubkeyHash::all_zeros())
          },
        })
        .collect(),
    };
